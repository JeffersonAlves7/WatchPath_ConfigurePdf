...
class Pdf:
    ...
    def separarPdf(self, save_local = f"./src/public/paginas/page"):
...
class Path:
    def readNotasEtiquetas():
        return os.listdir(f"{os.path.dirname(__file__)}\\public\\notas-etiquetas")
    def readPdfCaminho(caminho):
        return os.listdir(f"{os.path.dirname(__file__)}\\public\\notas-etiquetas\\{caminho}")
    def readPdfCaminhoFiles(path, file):
        return f"./src/public/notas-etiquetas/{path}/{file}"
    ...
    def eraseNotasEtiquetas():
        for i in os.listdir("./src/public/notas-etiquetas"):
            shutil.rmtree("./src/public/notas-etiquetas/" + i, ignore_errors=False, onerror=None)
        return 0

class NotaFiscal:
    def __init__(self):
        self.caminho = "./src/public/paginas/nfs"
        pass
...
class Etiqueta:
    def __init__(self):
        self.caminho = "./src/public/paginas/etqs"
        pass
    ...
    if len(Path.readNotasEtiquetas()) > 0:
        for i in range(0, len(Path.readNotasEtiquetas())):              #Passando por todas as pastas
            ...
            for i in os.listdir(CAMINHOS[0]):
                nf = Pdf(CAMINHOS[0] + "/" + i)
                Pdf.juntarPdf(i, "./src/public/pdfs/" + NotaFiscal.Number( nf.readPdf() ) + ".pdf") #já inserindo o nome certo
